...
local CFG = {
    ...
    skeleton_team_check = false,
    skeleton_thickness = 2,
    no_recoil = false,
}

local function zeroModule(m)
    if type(m) ~= "table" then return end
    pcall(function()
        m.CameraRecoilingEnabled = false
        m.Recoil = 0
        m.AngleX_Min = 0; m.AngleX_Max = 0
        m.AngleY_Min = 0; m.AngleY_Max = 0
        m.AngleZ_Min = 0; m.AngleZ_Max = 0
        m.XRecoil = 0; m.YRecoil = 0
        m.LeftValue = 0; m.RightValue = 0
        m.vmRecoil = Vector3.new(0, 0, 0)
        if type(m.recoil) == "table" then
            m.recoil.Recoil = 0
            m.recoil.AngleX_Min = 0; m.recoil.AngleX_Max = 0
            m.recoil.AngleY_Min = 0; m.recoil.AngleY_Max = 0
            m.recoil.AngleZ_Min = 0; m.recoil.AngleZ_Max = 0
        end
    end)
end

local function applyNoRecoilAll()
    local Guns = ReplicatedStorage:FindFirstChild("Modules") and ReplicatedStorage.Modules:FindFirstChild("Guns")
    if not Guns then return end
    for _, gunInst in Guns:GetChildren() do
        local ok, m = pcall(require, gunInst)
        if ok then zeroModule(m) end
    end
end
...
do
    local Guns = ReplicatedStorage:FindFirstChild("Modules") and ReplicatedStorage.Modules:FindFirstChild("Guns")
    if Guns then
        table.insert(conns, Guns.ChildAdded:Connect(function(child)
...
local function addSkeleton(plr)
    ...
    for i = 1, #boneMapR15 do
        ...
        l.Thickness = CFG.skeleton_thickness
        l.Color = CFG.skeleton_color
        l.Visible = false
        l.Transparency = 1
        lines[i] = l
    end
...
table.insert(conns, Players.PlayerRemoving:Connect(removeSkeleton))
...
local Window = Library:CreateWindow({ Title = "Lyzn Hub", SubTitle = "Convergence" })
local VisualsTab = Window:CreateTab("Visuals")
local CombatTab = Window:CreateTab("Combat")

local espSection = VisualsTab:CreateSection("Skeleton ESP", "Left")
espSection:AddToggle({ Text = "Skeleton", Default = false, Callback = function(v) CFG.skeleton_esp = v end })
espSection:AddToggle({ Text = "Team Check", Default = false, Callback = function(v) CFG.skeleton_team_check = v end })
espSection:AddSlider({ Text = "Thickness", Min = 1, Max = 5, Default = 2, Decimals = 1, Callback = function(v) CFG.skeleton_thickness = v end })

local recoilSection = CombatTab:CreateSection("Recoil", "Left")
recoilSection:AddToggle({ Text = "No Recoil", Default = false, Callback = function(v)
    CFG.no_recoil = v
    if v then pcall(applyNoRecoilAll) end
end })
